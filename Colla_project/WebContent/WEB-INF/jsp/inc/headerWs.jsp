...
// 		alert("headerWs.jsp의 stompClient는 정상connect 되었습니다");
		<%----------------------------------------채팅메시지 구독부분----------------------------------------------%>
		var crNum = $("#crNum").val();
		//일반메세지 구독
		stompClient.subscribe("/category/msg/"+crNum,function(){
				console.log("일반메세지 구독하였습니다");
		...
		//파일메세지 구독
		stompClient.subscribe("/category/file/"+crNum,function(){
			console.log("파일메세지 구독하였습니다");
				msgInfo = JSON.parse(cm.body);
				addMsg(msgInfo);
				$("#chatArea").scrollTop($("#chatArea")[0].scrollHeight);
		});
		...
		//코드메시지 구독
		stompClient.subscribe("/category/code/"+crNum,function(){
			console.log("코드메세지 구독하였습니다");
				msgInfo = JSON.parse(cm.body);
				addMsg(msgInfo);
				$("#chatArea").scrollTop($("#chatArea")[0].scrollHeight);
		});
		...
		//map메시지 구독
		stompClient.subscribe("/category/map/"+crNum,function(){
			console.log("map메세지 구독하였습니다");
				msgInfo = JSON.parse(cm.body);
				addMsg(msgInfo);
		...
		var mNum = $("#mNum").val();
		stompClient.subscribe("/category/loginMsg/"+mNum,function(){
			alert("로그인 요청 시도가 있었습니다.");
			console.log("로그인 요청 시도 발생");
			location.href="main";
		});
